...
import 'chartjs-adapter-moment' // Import the time adapter
...
export default class BubbleTradeChart {
  ...
  processTrade(data) {
    ...
    const newTrade = { x: time, y: price, r: this.getRadius(volume), q: volume, orders: 1, side }
  ...
  setupWebSocket() {
    ...
    this.socket.onopen = () => this.log(`WebSocket connected to ${this.config.symbol} aggTrade`, 'info')
